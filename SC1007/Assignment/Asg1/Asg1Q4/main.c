...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList)
{
    ...
    if(ll->size == 1){
        ...
        return;
    } else if (ll->size %2 != 0){   //odd
        divider = ((ll->size)/2)+1;
    } else {
        divider = (ll->size)/2;
    }
    //For front list
	for(int i=0; i<divider; i++, cur=cur->next){
        newNode = malloc(sizeof(ListNode));
        newNode->item = cur->item;
        newNode->next = NULL;
        if(i==0){
            resultFrontList->head = newNode;
            frontL = resultFrontList->head;
        } else {
            frontL->next = newNode;
            frontL = frontL->next;
        }
	...
	//For back list
	for(int x=0; cur!=NULL; x++, cur=cur->next){
        newNode = malloc(sizeof(ListNode));
        newNode->item = cur->item;
        newNode->next = NULL;
        if(x==0){
            resultBackList->head = newNode;
            backL = resultBackList->head;
        }else {
            backL->next = newNode;
            backL = backL->next;
        }
