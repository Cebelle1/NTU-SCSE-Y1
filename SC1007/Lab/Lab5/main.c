...
int palindrome(char *word){
	...
	//printf("size: %d\n",(sizeof(word)-1)*4);
	for (int i=0; i< (sizeof(word)-1)*4; i++){
            printf("%d ", i);
        if(word[i] == NULL || word[i] ==' '){
            //printf("Continuing at: %c\n", word[i]);
            continue;
        } else {
            //printf("Successful insert: %c\n", word[i]);
            if(word[i] < 97){
                smallcase = word[i] + 32;
                //printf("Smallcase: %c\n", smallcase);
                push(&s, smallcase);
	...
	for (int i=0; i< (s.ll).size; i++){
        ...
        stackret = pop(&s);
        queueret = dequeue(&q);
        //printf("stackret: %c and queueret %c\n", stackret, queueret);
        if(stackret == queueret){
                //printf("itemret: %d", itemret);
            continue;
        } else {
