...
void adjM2adjL(Graph *g)
{
    ...
    //The graph g can only use either list or matrix at 1 time. overridding occurs if trying to convert without creating dummy
    //Overriding > write into list simultaneously changes the matrix value. Colliding addresses spaces (?)
    //Create new list with matrix first then replace at the end
